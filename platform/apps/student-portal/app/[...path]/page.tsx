...
export default async function PortalRoute({ params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (!(Object.values(portalViewRoutes.student) as string[]).includes(initialPath)) notFound();
  return <PortalHome portal={portalDefinitions.student} initialPath={initialPath} release={process.env.VERCEL_GIT_COMMIT_SHA} />;
}
