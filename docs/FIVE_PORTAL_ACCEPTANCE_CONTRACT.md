...
One seeded synthetic institution contains one complete CSE storyline with one
HOD, one assigned faculty member, one student, one linked parent, one governance
operator, and one offering. A minimal ECE fixture exists only to prove
...
6. Parent completes one sandbox fee payment and downloads its synthetic receipt.
7. A committed academic event creates one outbox item.
8. The worker freezes evidence, evaluates deterministic policy, and produces a
   cited proposal through the LLM or deterministic fallback.
9. Faculty approves or rejects the exact current artifact.
10. Approved support becomes visible to the student and, when permitted, parent.
11. HOD sees the departmental disposition.
12. AI Governance shows lineage and verifies side-effect-free replay.
...
| Parent | Read active, field-permitted linked-student data and complete sandbox payment | Revoked field and unlinked student fail closed |
| Faculty | Read assigned roster, submit owned attendance and marks, decide owned case | Unassigned section and stale-artifact approval fail closed |
...
The seeded journey passes three consecutive runs after a clean reset. Any
