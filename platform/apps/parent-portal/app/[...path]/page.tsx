...
export default async function PortalRoute({ params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (!(Object.values(portalViewRoutes.parent) as string[]).includes(initialPath)) notFound();
  return <PortalHome portal={portalDefinitions.parent} initialPath={initialPath} release={process.env.VERCEL_GIT_COMMIT_SHA} />;
}
